...
SctpTransport::SctpTransport(
    ...
    : owner_thread_(rtc::Thread::Current()),
      info_(libmedia_transfer_protocol::SctpTransportState::kNew),
  ...
  if (dtls_transport_) {
    UpdateInformation(libmedia_transfer_protocol::SctpTransportState::kConnecting);
  } else {
    UpdateInformation(libmedia_transfer_protocol::SctpTransportState::kNew);
  }
}
...
libmedia_transfer_protocol::SctpTransportInformation SctpTransport::Information() const {
  ...
  if (!owner_thread_->IsCurrent()) {
    return owner_thread_->Invoke<libmedia_transfer_protocol::SctpTransportInformation>(
...
void SctpTransport::RegisterObserver(libmedia_transfer_protocol::SctpTransportObserverInterface* observer) {
  RTC_DCHECK_RUN_ON(owner_thread_);
  RTC_DCHECK(observer);
...
rtc::scoped_refptr<libice::DtlsTransportInterface> SctpTransport::dtls_transport()
    const {
  RTC_DCHECK_RUN_ON(owner_thread_);
...
void SctpTransport::Clear() {
  ...
  UpdateInformation(libmedia_transfer_protocol::SctpTransportState::kClosed);
}

void SctpTransport::SetDtlsTransport(
    rtc::scoped_refptr<DtlsTransport> transport) {
  RTC_DCHECK_RUN_ON(owner_thread_);
  libmedia_transfer_protocol::SctpTransportState next_state = info_.state();
  ...
  if (internal_sctp_transport_) {
    if (transport) {
      ...
      if (info_.state() == libmedia_transfer_protocol::SctpTransportState::kNew) {
        next_state = libmedia_transfer_protocol::SctpTransportState::kConnecting;
      }
    } else {
...
void SctpTransport::Start(int local_port,
  ...
  RTC_DCHECK_RUN_ON(owner_thread_);
  info_ = libmedia_transfer_protocol::SctpTransportInformation(info_.state(), info_.dtls_transport(),
...
void SctpTransport::UpdateInformation(libmedia_transfer_protocol::SctpTransportState state) {
  RTC_DCHECK_RUN_ON(owner_thread_);
  ...
  if (internal_sctp_transport_) {
    info_ = libmedia_transfer_protocol::SctpTransportInformation(
...
void SctpTransport::OnAssociationChangeCommunicationUp() {
  ...
  UpdateInformation(libmedia_transfer_protocol::SctpTransportState::kConnected);
}

void SctpTransport::OnDtlsStateChange(libice::DtlsTransportInternal* transport,
                                      libice::DtlsTransportState state) {
  RTC_DCHECK_RUN_ON(owner_thread_);
  RTC_CHECK(transport == dtls_transport_->internal());
  if (state == libice::DtlsTransportState::kClosed ||
      state == libice::DtlsTransportState::kFailed) {
    UpdateInformation(libmedia_transfer_protocol::SctpTransportState::kClosed);
