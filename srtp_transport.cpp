...
#include "libp2p_peerconnection/srtp_session.h"
...
webrtc::RTCError SrtpTransport::SetSrtpSendKey(const cricket::CryptoParams& params) {
  if (send_params_) 
  { 
...
webrtc::RTCError SrtpTransport::SetSrtpReceiveKey(const cricket::CryptoParams& params) {
  if (recv_params_) {
    LOG_AND_RETURN_ERROR(
        webrtc::RTCErrorType::UNSUPPORTED_OPERATION,
        "Setting the SRTP send key twice is currently unsupported.");
...
#endif
  if (!res) {
    uint16_t seq_num = libmedia_transfer_protocol::ParseRtpSequenceNumber(*packet);
    uint32_t ssrc = libmedia_transfer_protocol::ParseRtpSsrc(*packet);
...
bool SrtpTransport::SendRtcpPacket(rtc::CopyOnWriteBuffer* packet,
  ...
  if (!ProtectRtcp(data, len, static_cast<int>(packet->capacity()), &len)) {
    int type = -1;
	libmedia_transfer_protocol::GetRtcpType(data, len, &type);
  ...
  if (!UnprotectRtp(data, len, &len)) {
    ...
    if (decryption_failure_count_ % kFailureLogThrottleCount == 0) {
      RTC_LOG(LS_ERROR) << "Failed to unprotect RTP packet: size=" << len
                        << ", seqnum=" << libmedia_transfer_protocol::ParseRtpSequenceNumber(packet)
                        << ", SSRC=" << libmedia_transfer_protocol::ParseRtpSsrc(packet)
