...
#include "libp2p_peerconnection/engine/webrtc_video_engine.h"
...
const char* StreamTypeToString(
	libp2p_peerconnection::VideoSendStream::StreamStats::StreamType type) {
  switch (type) {
    case libp2p_peerconnection::VideoSendStream::StreamStats::StreamType::kMedia:
      return "kMedia";
    case libp2p_peerconnection::VideoSendStream::StreamStats::StreamType::kRtx:
      return "kRtx";
    case libp2p_peerconnection::VideoSendStream::StreamStats::StreamType::kFlexfec:
      return "kFlexfec";
  }
  return nullptr;
}
...
bool IsScaleFactorsPowerOfTwo(const libmedia_codec::VideoEncoderConfig& config) {
...
void AddDefaultFeedbackParams(libmedia_transfer_protocol::VideoCodec* codec/*,
                              const webrtc::WebRtcKeyValueConfig& trials*/) {
  // Don't add any feedback params for RED and ULPFEC.
  if (codec->name == libmedia_transfer_protocol::kRedCodecName || codec->name == libmedia_transfer_protocol::kUlpfecCodecName)
    return;
  codec->AddFeedbackParam(libmedia_transfer_protocol::FeedbackParam(libmedia_transfer_protocol::kRtcpFbParamRemb, libmedia_transfer_protocol::kParamValueEmpty));
  codec->AddFeedbackParam(
	  libmedia_transfer_protocol:: FeedbackParam(libmedia_transfer_protocol::kRtcpFbParamTransportCc
		  , libmedia_transfer_protocol::kParamValueEmpty));
  // Don't add any more feedback params for FLEXFEC.
  if (codec->name == libmedia_transfer_protocol::kFlexfecCodecName)
    return;
  codec->AddFeedbackParam(libmedia_transfer_protocol::FeedbackParam(libmedia_transfer_protocol::kRtcpFbParamCcm, libmedia_transfer_protocol::kRtcpFbCcmParamFir));
  codec->AddFeedbackParam(libmedia_transfer_protocol::FeedbackParam(libmedia_transfer_protocol::kRtcpFbParamNack,libmedia_transfer_protocol:: kParamValueEmpty));
  codec->AddFeedbackParam(libmedia_transfer_protocol::FeedbackParam(libmedia_transfer_protocol::kRtcpFbParamNack,libmedia_transfer_protocol:: kRtcpFbNackParamPli));
  /*if (codec->name == libmedia_transfer_protocol::kVp8CodecName &&
      IsEnabled(trials, "WebRTC-RtcpLossNotification")) {
    codec->AddFeedbackParam(libmedia_transfer_protocol::FeedbackParam(libmedia_transfer_protocol::kRtcpFbParamLntf, libmedia_transfer_protocol::kParamValueEmpty));
  }*/
}
...
template <class T>
std::vector<libmedia_transfer_protocol::VideoCodec> GetPayloadTypesAndDefaultCodecs(
    const T* factory,
  ...
  std::vector<libmedia_codec::SdpVideoFormat> supported_formats = factory->GetSupportedFormats();
  if (is_decoder_factory)
  {
//	  using namespace lilbmedia_transfer_protocol;
	//  AddH264ConstrainedBaselineProfileToSupportedFormats(&supported_formats);
  }

  if (supported_formats.empty())
    return std::vector<libmedia_transfer_protocol::VideoCodec>();

  supported_formats.push_back(libmedia_codec::SdpVideoFormat(libmedia_transfer_protocol::kRedCodecName));
  supported_formats.push_back(libmedia_codec::SdpVideoFormat(libmedia_transfer_protocol::kUlpfecCodecName));
  ...
  std::vector<libmedia_transfer_protocol::VideoCodec> output_codecs;
  for (const libmedia_codec::SdpVideoFormat& format : supported_formats) {
    libmedia_transfer_protocol::VideoCodec codec(format);
    bool isCodecValidForLowerRange =
        absl::EqualsIgnoreCase(codec.name, libmedia_transfer_protocol::kFlexfecCodecName) ||
        absl::EqualsIgnoreCase(codec.name, libmedia_transfer_protocol::kAv1CodecName) ||
        absl::EqualsIgnoreCase(codec.name, libmedia_transfer_protocol::kAv1xCodecName);
    bool isFecCodec = absl::EqualsIgnoreCase(codec.name, libmedia_transfer_protocol::kUlpfecCodecName) ||
                      absl::EqualsIgnoreCase(codec.name, libmedia_transfer_protocol::kFlexfecCodecName);
    ...
    if (!isFecCodec) {
      ...
      if (isCodecValidForLowerRange ||
        ...
        output_codecs.push_back(
			libmedia_transfer_protocol::VideoCodec::CreateRtxCodec(payload_type_lower++, codec.id));
      } else {
        output_codecs.push_back(
			libmedia_transfer_protocol::VideoCodec::CreateRtxCodec(payload_type_upper++, codec.id));
      }
    }
  }
...
bool IsTemporalLayersSupported(const std::string& codec_name) {
  return absl::EqualsIgnoreCase(codec_name, libmedia_transfer_protocol::kVp8CodecName) ||
         absl::EqualsIgnoreCase(codec_name, libmedia_transfer_protocol::kVp9CodecName);
}

static std::string CodecVectorToString(const std::vector<libmedia_transfer_protocol::VideoCodec>& codecs) {
...
static bool ValidateCodecFormats(const std::vector<libmedia_transfer_protocol::VideoCodec>& codecs) {
  bool has_video = false;
  for (size_t i = 0; i < codecs.size(); ++i) {
    if (!codecs[i].ValidateCodecFormat()) {
      return false;
    }
    if (codecs[i].GetCodecType() == libmedia_transfer_protocol::VideoCodec::CODEC_VIDEO) {
      has_video = true;
    }
  }
  if (!has_video) {
...
static bool ValidateStreamParams(const libmedia_transfer_protocol::StreamParams& sp) {
  if (sp.ssrcs.empty()) {
...
bool IsCodecDisabledForSimulcast(const std::string& codec_name/*,
                                 const webrtc::WebRtcKeyValueConfig& trials*/) {
	return  true;  /*!absl::StartsWith(trials.Lookup("WebRTC-H264Simulcast"), "Disabled")
             ? absl::EqualsIgnoreCase(codec_name,libmedia_transfer_protocol:: kVp9CodecName)
             : absl::EqualsIgnoreCase(codec_name, libmedia_transfer_protocol::kH264CodecName) ||
                   absl::EqualsIgnoreCase(codec_name, libmedia_transfer_protocol::kVp9CodecName);*/
}
...
bool IsLayerActive(const libmedia_transfer_protocol::RtpEncodingParameters& layer) {
  return layer.active &&
...
size_t FindRequiredActiveLayers(
    const libmedia_codec::VideoEncoderConfig& encoder_config) {
...
int NumActiveStreams(const libmedia_transfer_protocol::RtpParameters& rtp_parameters) {
  int res = 0;
...
MergeInfoAboutOutboundRtpSubstreams(
    const std::map<uint32_t,  VideoSendStream::StreamStats>&
        substreams) {
  ...
  for (const auto& pair : substreams) {
    ...
    const  VideoSendStream::StreamStats& associated_substream =
        pair.second;
    ...
    if (substreams.find(media_ssrc) == substreams.end()) {
     ...
     VideoSendStream::StreamStats& rtp_substream =
...
rtc::scoped_refptr<libmedia_codec::VideoEncoderConfig::EncoderSpecificSettings>
WebRtcVideoChannel::WebRtcVideoSendStream::ConfigureVideoEncoderSettings(
    const libmedia_transfer_protocol::VideoCodec& codec) {
  ...
  if (absl::EqualsIgnoreCase(codec.name, libmedia_transfer_protocol::kH264CodecName)) {
    libmedia_codec::VideoCodecH264 h264_settings =
		libmedia_codec::VideoEncoder::GetDefaultH264Settings();
    h264_settings.frameDroppingOn = frame_dropping;
    return rtc::make_ref_counted<
		libmedia_codec::VideoEncoderConfig::H264EncoderSpecificSettings>(h264_settings);
  }
  if (absl::EqualsIgnoreCase(codec.name, libmedia_transfer_protocol::kVp8CodecName)) {
	  libmedia_codec::VideoCodecVP8 vp8_settings =
		  libmedia_codec::VideoEncoder::GetDefaultVp8Settings();
    vp8_settings.automaticResizeOn = automatic_resize;
    // VP8 denoising is enabled by default.
    vp8_settings.denoisingOn = codec_default_denoising ? true : denoising;
    vp8_settings.frameDroppingOn = frame_dropping;
    return rtc::make_ref_counted<
		libmedia_codec::VideoEncoderConfig::Vp8EncoderSpecificSettings>(vp8_settings);
  }
  if (absl::EqualsIgnoreCase(codec.name, libmedia_transfer_protocol::kVp9CodecName)) {
	  libmedia_codec::VideoCodecVP9 vp9_settings =
		  libmedia_codec::VideoEncoder::GetDefaultVp9Settings();
    const size_t default_num_spatial_layers =
        parameters_.config.rtp.ssrcs.size();
	const size_t num_spatial_layers = 0;
    ...
    const size_t default_num_temporal_layers =
        num_spatial_layers > 1 ? libmedia_transfer_protocol::kConferenceDefaultNumTemporalLayers : 1;
...
*/
    vp9_settings.numberOfSpatialLayers = std::min<unsigned char>(
        num_spatial_layers, libmedia_transfer_protocol::kConferenceMaxNumSpatialLayers);
    vp9_settings.numberOfTemporalLayers = std::min<unsigned char>(
        num_temporal_layers, libmedia_transfer_protocol::kConferenceMaxNumTemporalLayers);
    ...
    //} else
	{
      ...
      vp9_settings.interLayerPred = libmedia_codec::InterLayerPredMode::kOn;
    }
    return rtc::make_ref_counted<
		libmedia_codec::VideoEncoderConfig::Vp9EncoderSpecificSettings>(vp9_settings);
  }
  return nullptr;
}
...
UnsignalledSsrcHandler::Action DefaultUnsignalledSsrcHandler::OnUnsignalledSsrc(
  ...
  libmedia_transfer_protocol::StreamParams sp = channel->unsignaled_stream_params();
...
libmedia_codec::VideoSinkInterface<libmedia_codec::VideoFrame>*
...
void DefaultUnsignalledSsrcHandler::SetDefaultSink(
    WebRtcVideoChannel* channel,
	libmedia_codec::VideoSinkInterface<libmedia_codec::VideoFrame>* sink) {
  default_sink_ = sink;
  absl::optional<uint32_t> default_recv_ssrc =
      channel->GetDefaultReceiveStreamSsrc();
  if (default_recv_ssrc) {
...
libmedia_transfer_protocol::VideoMediaChannel* WebRtcVideoEngine::CreateMediaChannel(
    //webrtc::Call* call,
...
std::vector<libmedia_transfer_protocol::VideoCodec> WebRtcVideoEngine::send_codecs() const {
  return GetPayloadTypesAndDefaultCodecs(encoder_factory_.get(),
                                         /*is_decoder_factory=*/false/*, trials_*/);
}

std::vector<libmedia_transfer_protocol::VideoCodec> WebRtcVideoEngine::recv_codecs() const {
  return GetPayloadTypesAndDefaultCodecs(decoder_factory_.get(),
                                         /*is_decoder_factory=*/true/*, trials_*/);
}

std::vector<libmedia_transfer_protocol::RtpHeaderExtensionCapability>
WebRtcVideoEngine::GetRtpHeaderExtensions() const {
  std::vector<libmedia_transfer_protocol::RtpHeaderExtensionCapability> result;
  int id = 1;
  for (const auto& uri :
       {libmedia_transfer_protocol::RtpExtension::kTimestampOffsetUri,
        libmedia_transfer_protocol::RtpExtension::kAbsSendTimeUri,
        libmedia_transfer_protocol::RtpExtension::kVideoRotationUri,
        libmedia_transfer_protocol::RtpExtension::kTransportSequenceNumberUri,
        libmedia_transfer_protocol::RtpExtension::kPlayoutDelayUri,
        libmedia_transfer_protocol::RtpExtension::kVideoContentTypeUri,
        libmedia_transfer_protocol::RtpExtension::kVideoTimingUri,
        libmedia_transfer_protocol::RtpExtension::kColorSpaceUri, libmedia_transfer_protocol::RtpExtension::kMidUri,
        libmedia_transfer_protocol::RtpExtension::kRidUri, libmedia_transfer_protocol::RtpExtension::kRepairedRidUri}) {
    result.emplace_back(uri, id++, libmedia_transfer_protocol::RtpTransceiverDirection::kSendRecv);
  }
  result.emplace_back(libmedia_transfer_protocol::RtpExtension::kGenericFrameDescriptorUri00, id++,
                      IsEnabled(trials_, "WebRTC-GenericDescriptorAdvertised")
                          ? libmedia_transfer_protocol::RtpTransceiverDirection::kSendRecv
                          : libmedia_transfer_protocol::RtpTransceiverDirection::kStopped);
  result.emplace_back(
	  libmedia_transfer_protocol::RtpExtension::kDependencyDescriptorUri, id++,
      IsEnabled(trials_, "WebRTC-DependencyDescriptorAdvertised")
          ? libmedia_transfer_protocol::RtpTransceiverDirection::kSendRecv
          : libmedia_transfer_protocol::RtpTransceiverDirection::kStopped);

  result.emplace_back(
	  libmedia_transfer_protocol::RtpExtension::kVideoLayersAllocationUri, id++,
      IsEnabled(trials_, "WebRTC-VideoLayersAllocationAdvertised")
          ? libmedia_transfer_protocol::RtpTransceiverDirection::kSendRecv
          : libmedia_transfer_protocol::RtpTransceiverDirection::kStopped);

  result.emplace_back(
	  libmedia_transfer_protocol::RtpExtension::kVideoFrameTrackingIdUri, id++,
...
WebRtcVideoChannel::WebRtcVideoChannel(
    ...
    const libmedia_transfer_protocol::VideoOptions& options,
    const libmedia_transfer_protocol::CryptoOptions& crypto_options,
    libmedia_codec::VideoEncoderFactory* encoder_factory,
    libmedia_codec::VideoDecoderFactory* decoder_factory,
    libmedia_codec::VideoBitrateAllocatorFactory* bitrate_allocator_factory)
    : VideoMediaChannel(config, nullptr/* call->network_thread()*/),
      worker_thread_(nullptr/*call->worker_thread()*/),
      //call_(call),
      ...
      discard_unknown_ssrc_packets_( false /*
          IsEnabled(call_->trials(),
                    "WebRTC-Video-DiscardPacketsWithUnknownSsrc")*/),
      crypto_options_(crypto_options),
      unknown_ssrc_packet_buffer_(new UnhandledPacketsBuffer()) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  ...
  recv_codecs_ = MapCodecs(GetPayloadTypesAndDefaultCodecs(
      decoder_factory_, /*is_decoder_factory=*/true/*, trials*//*call_->trials()*/));
...
WebRtcVideoChannel::SelectSendVideoCodecs(
  ...
  std::vector<libmedia_codec::SdpVideoFormat> sdp_formats =
      encoder_factory_ ? encoder_factory_->GetImplementations()
                       : std::vector<libmedia_codec::SdpVideoFormat>();
...
bool WebRtcVideoChannel::GetChangedSendParameters(
    const libmedia_transfer_protocol::VideoSendParameters& params,
  ...
  std::vector<libmedia_transfer_protocol::RtpExtension> filtered_extensions = FilterRtpExtensions(
      params.extensions, libmedia_transfer_protocol::RtpExtension::IsSupportedForVideo, true/*,
      call_->trials()*/);
  if (!send_rtp_extensions_ || (*send_rtp_extensions_ != filtered_extensions)) {
    changed_params->rtp_header_extensions =
        absl::optional<std::vector<libmedia_transfer_protocol::RtpExtension>>(filtered_extensions);
  }
  ...
  if (params.rtcp.reduced_size != send_params_.rtcp.reduced_size) {
    changed_params->rtcp_mode = params.rtcp.reduced_size
                                    ? libmedia_transfer_protocol::RtcpMode::kReducedSize
                                    : libmedia_transfer_protocol::RtcpMode::kCompound;
  }

  return true;
}

bool WebRtcVideoChannel::SetSendParameters(const libmedia_transfer_protocol::VideoSendParameters& params) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
...
void WebRtcVideoChannel::RequestEncoderSwitch(
    const libmedia_codec::SdpVideoFormat& format) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  ...
  if (changed_params.send_codec || changed_params.rtcp_mode) {
    ...
    for (auto& kv : receive_streams_) {
      ...
      kv.second->SetFeedbackParameters(
          ...
          send_params_.rtcp.reduced_size ? libmedia_transfer_protocol::RtcpMode::kReducedSize
                                         : libmedia_transfer_protocol::RtcpMode::kCompound,
...
libmedia_transfer_protocol::RtpParameters WebRtcVideoChannel::GetRtpSendParameters(
    uint32_t ssrc) const {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  ...
  if (it == send_streams_.end()) {
    ...
    return libmedia_transfer_protocol::RtpParameters();
  }

  libmedia_transfer_protocol::RtpParameters rtp_params = it->second->GetRtpParameters();
  // Need to add the common list of codecs to the send stream-specific
  // RTP parameters.
  for (const libmedia_transfer_protocol::VideoCodec& codec : send_params_.codecs) {
...
webrtc::RTCError WebRtcVideoChannel::SetRtpSendParameters(
    uint32_t ssrc,
    const libmedia_transfer_protocol::RtpParameters& parameters) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  ...
  libmedia_transfer_protocol::RtpParameters current_parameters = GetRtpSendParameters(ssrc);
...
libmedia_transfer_protocol::RtpParameters WebRtcVideoChannel::GetRtpReceiveParameters(
    uint32_t ssrc) const {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  libmedia_transfer_protocol::RtpParameters rtp_params;
  ...
  for (const libmedia_transfer_protocol::VideoCodec& codec : recv_params_.codecs) {
    rtp_params.codecs.push_back(codec.ToCodecParameters());
  }
...
libmedia_transfer_protocol::RtpParameters WebRtcVideoChannel::GetDefaultRtpReceiveParameters()
    const {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  libmedia_transfer_protocol::RtpParameters rtp_params;
...
bool WebRtcVideoChannel::GetChangedRecvParameters(
    const libmedia_transfer_protocol::VideoRecvParameters& params,
  ...
  if (params.is_stream_active) {
    const std::vector<libmedia_transfer_protocol::VideoCodec> local_supported_codecs =
  ...
  // Handle RTP header extensions.
  std::vector<libmedia_transfer_protocol::RtpExtension> filtered_extensions = FilterRtpExtensions(
      params.extensions, libmedia_transfer_protocol::RtpExtension::IsSupportedForVideo, false/*,
      call_->trials()*/);
...
bool WebRtcVideoChannel::SetRecvParameters(const libmedia_transfer_protocol::VideoRecvParameters& params) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
...
bool WebRtcVideoChannel::GetSendCodec(libmedia_transfer_protocol::VideoCodec* codec) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  if (!send_codec_) {
...
bool WebRtcVideoChannel::ValidateSendSsrcAvailability(
    const libmedia_transfer_protocol::StreamParams& sp) const {
...
bool WebRtcVideoChannel::AddSendStream(const libmedia_transfer_protocol::StreamParams& sp) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  ...
  VideoSendStream::Config config(this);

  for (const  libmedia_transfer_protocol::RidDescription& rid : sp.rids()) {
...
//
//bool WebRtcVideoChannel::AddRecvStream(const libmedia_transfer_protocol::StreamParams& sp) {
...
void WebRtcVideoChannel::OnDemuxerCriteriaUpdateComplete() {
  ...
  worker_thread_->PostTask(webrtc::ToQueuedTask(task_safety_, [this] {
    RTC_DCHECK_RUN_ON(&thread_checker_);
...
bool WebRtcVideoChannel::GetStats(libmedia_transfer_protocol::VideoMediaInfo* info) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
...
void WebRtcVideoChannel::FillSenderStats(libmedia_transfer_protocol::VideoMediaInfo* video_media_info,
                                         bool log_stats) {
...
void WebRtcVideoChannel::FillReceiverStats(libmedia_transfer_protocol::VideoMediaInfo* video_media_info,
                                           bool log_stats) {
...
void WebRtcVideoChannel::FillBitrateInfo(libmedia_transfer_protocol::BandwidthEstimationInfo* bwe_info) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
...
void WebRtcVideoChannel::FillSendAndReceiveCodecStats(
	libmedia_transfer_protocol::VideoMediaInfo* video_media_info) {
  for (const libmedia_transfer_protocol::VideoCodec& codec : send_params_.codecs) {
	  libmedia_transfer_protocol::RtpCodecParameters codec_params = codec.ToCodecParameters();
...
void WebRtcVideoChannel::SetInterface(NetworkInterface* iface) {
  ...
  int recv_buffer_size = libmedia_transfer_protocol::kVideoRtpRecvBufferSize;
  if (!group_name_recv_buf_size.empty() &&
    ...
    recv_buffer_size = libmedia_transfer_protocol::kVideoRtpRecvBufferSize;
  }
  ...
  int send_buffer_size = libmedia_transfer_protocol::kVideoRtpSendBufferSize;
  if (!group_name_send_buf_size.empty() &&
    ...
    send_buffer_size = libmedia_transfer_protocol::kVideoRtpSendBufferSize;
  }
...
void WebRtcVideoChannel::SetFrameDecryptor(
    uint32_t ssrc,
    rtc::scoped_refptr<libmedia_transfer_protocol::FrameDecryptorInterface> frame_decryptor) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
...
std::vector<libmedia_transfer_protocol::RtpSource> WebRtcVideoChannel::GetSources(
    uint32_t ssrc) const {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  auto it = receive_streams_.find(ssrc);
  if (it == receive_streams_.end()) {
...
WebRtcVideoChannel::WebRtcVideoSendStream::WebRtcVideoSendStream(
    //webrtc::Call* call,
    const libmedia_transfer_protocol::StreamParams& sp,
     VideoSendStream::Config config,
    const libmedia_transfer_protocol::VideoOptions& options,
    bool enable_cpu_overuse_detection,
    int max_bitrate_bps,
    const absl::optional<VideoCodecSettings>& codec_settings,
    const absl::optional<std::vector<libmedia_transfer_protocol::RtpExtension>>& rtp_extensions,
    // TODO(deadbeef): Don't duplicate information between send_params,
    // rtp_extensions, options, etc.
    const libmedia_transfer_protocol::VideoSendParameters& send_params)
    : worker_thread_(nullptr/*call->worker_thread()*/),
      ssrcs_(sp.ssrcs),
     ...
     // call_(call),
      ...
      sending_(false),
      disable_automatic_resize_(true/*
  ...
  parameters_.config.rtp.max_packet_size =
      std::min<size_t>(parameters_.config.rtp.max_packet_size, libmedia_transfer_protocol::kVideoMtu);
...
bool WebRtcVideoChannel::WebRtcVideoSendStream::SetVideoSend(
  ...
  if (options) {
	  libmedia_transfer_protocol::VideoOptions old_options = parameters_.options;
  ...
  if (source_ && stream_) {
    stream_->SetSource(nullptr, libmedia_transfer_protocol::DegradationPreference::DISABLED);
  }
...
libmedia_transfer_protocol::DegradationPreference
WebRtcVideoChannel::WebRtcVideoSendStream::GetDegradationPreference() const {
  ...
  if (!enable_cpu_overuse_detection_) {
    return libmedia_transfer_protocol::DegradationPreference::DISABLED;
  }

  libmedia_transfer_protocol::DegradationPreference degradation_preference;
  if (rtp_parameters_.degradation_preference.has_value()) {
    ...
    if (parameters_.options.content_hint ==
		libmedia_transfer_protocol::VideoTrackInterface::ContentHint::kFluid) {
      degradation_preference =
		  libmedia_transfer_protocol::DegradationPreference::MAINTAIN_FRAMERATE;
	}
	else if (parameters_.options.is_screencast.value_or(false) ||
		parameters_.options.content_hint ==
		libmedia_transfer_protocol::VideoTrackInterface::ContentHint::kDetailed ||
		parameters_.options.content_hint ==
		libmedia_transfer_protocol::VideoTrackInterface::ContentHint::kText) {
		degradation_preference =
			libmedia_transfer_protocol::DegradationPreference::MAINTAIN_RESOLUTION;
		...
		//  degradation_preference = libmedia_transfer_protocol::DegradationPreference::BALANCED;
		//} 
	}
	else {
...
void WebRtcVideoChannel::WebRtcVideoSendStream::SetCodec(
  ...
  parameters_.config.rtp.raw_payload =
      codec_settings.codec.packetization == libmedia_transfer_protocol::kPacketizationParamRaw;
...
void WebRtcVideoChannel::WebRtcVideoSendStream::SetSendParameters(
  ...
  if (params.rtcp_mode) {
    ...
    rtp_parameters_.rtcp.reduced_size =
        parameters_.config.rtp.rtcp_mode == libmedia_transfer_protocol::RtcpMode::kReducedSize;
...
libmedia_transfer_protocol::RtpParameters
...
libmedia_codec::VideoEncoderConfig
WebRtcVideoChannel::WebRtcVideoSendStream::CreateVideoEncoderConfig(
    const libmedia_transfer_protocol::VideoCodec& codec) const {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  libmedia_codec::VideoEncoderConfig encoder_config;
  encoder_config.codec_type = libmedia_codec::PayloadStringToCodecType(codec.name);
  encoder_config.video_format =
	  libmedia_codec::SdpVideoFormat(codec.name, codec.params);
  ...
  if (is_screencast) {
    ...
    encoder_config.content_type =
		libmedia_codec::VideoEncoderConfig::ContentType::kScreen;
  } else {
    encoder_config.min_transmit_bitrate_bps = 0;
    encoder_config.content_type =
		libmedia_codec::VideoEncoderConfig::ContentType::kRealtimeVideo;
  }
  ...
  if (codec.GetParam(libmedia_transfer_protocol::kCodecParamMaxBitrate, &codec_max_bitrate_kbps) &&
  ...
  codec.GetParam(libmedia_transfer_protocol::kCodecParamMaxQuantization, &max_qp);
...
std::vector<libmedia_transfer_protocol::VideoSenderInfo>
WebRtcVideoChannel::WebRtcVideoSendStream::GetPerLayerVideoSenderInfos(
    bool log_stats) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  libmedia_transfer_protocol::VideoSenderInfo common_info;
  ...
  std::vector<libmedia_transfer_protocol::VideoSenderInfo> infos;
...
libmedia_transfer_protocol::VideoSenderInfo
WebRtcVideoChannel::WebRtcVideoSendStream::GetAggregatedVideoSenderInfo(
    const std::vector<libmedia_transfer_protocol::VideoSenderInfo>& infos) const {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  RTC_CHECK(!infos.empty());
  if (infos.size() == 1) {
    return infos[0];
  }
  libmedia_transfer_protocol::VideoSenderInfo info = infos[0];
...
void WebRtcVideoChannel::WebRtcVideoSendStream::FillBitrateInfo(
	libmedia_transfer_protocol:: BandwidthEstimationInfo* bwe_info) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  if (stream_ == NULL) {
    return;
  }
...
void WebRtcVideoChannel::WebRtcVideoSendStream::
    SetEncoderToPacketizerFrameTransformer(
        rtc::scoped_refptr<libmedia_transfer_protocol::FrameTransformerInterface>
            frame_transformer) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
...
std::vector<libmedia_transfer_protocol::RtpSource>
...
WebRtcVideoChannel::WebRtcVideoReceiveStream::GetRtpParameters() const {
	libmedia_transfer_protocol::RtpParameters rtp_parameters;
 ...
 /* rtp_parameters.header_extensions = config_.rtp.extensions;
  rtp_parameters.rtcp.reduced_size =
      config_.rtp.rtcp_mode == libmedia_transfer_protocol::RtcpMode::kReducedSize;
*/
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::SetFeedbackParameters(
    ...
    libmedia_transfer_protocol::RtcpMode rtcp_mode,
    int rtx_time) {
  int nack_history_ms =
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::OnFrame(
    const libmedia_codec::VideoFrame& frame) {
...
libmedia_transfer_protocol::VideoReceiverInfo
WebRtcVideoChannel::WebRtcVideoReceiveStream::GetVideoReceiverInfo(
    bool log_stats) {
	libmedia_transfer_protocol::VideoReceiverInfo info;
 ...
 /* info.packets_rcvd = stats.rtp_stats.packet_counter.packets;
  ...
  info.jitter_ms = stats.rtp_stats.jitter / (libmedia_transfer_protocol::kVideoCodecClockrate / 1000);
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::
    SetRecordableEncodedFrameCallback(
        std::function<void(const libmedia_codec::RecordableEncodedFrame&)> callback) {
  //if (stream_) {
...
std::vector<WebRtcVideoChannel::VideoCodecSettings>
WebRtcVideoChannel::MapCodecs(const std::vector<libmedia_transfer_protocol::VideoCodec>& codecs) {
  if (codecs.empty()) {
    return {};
  }

  std::vector<VideoCodecSettings> video_codecs;
  std::map<int,  libmedia_transfer_protocol::VideoCodec::CodecType > payload_codec_type;
  ...
  for (const libmedia_transfer_protocol::VideoCodec& in_codec : codecs) {
	...
	case libmedia_transfer_protocol::VideoCodec::CODEC_RED: {
	...
	case libmedia_transfer_protocol::VideoCodec::CODEC_ULPFEC: {
      ...
      case libmedia_transfer_protocol::VideoCodec::CODEC_FLEXFEC: {
      ...
      case libmedia_transfer_protocol::VideoCodec::CODEC_RTX: {
        int associated_payload_type;
        if (!in_codec.GetParam(libmedia_transfer_protocol::kCodecParamAssociatedPayloadType,
                               &associated_payload_type) ||
            !libmedia_transfer_protocol::IsValidRtpPayloadType(associated_payload_type)) {
          RTC_LOG(LS_ERROR)
        ...
        int rtx_time;
        if (in_codec.GetParam(libmedia_transfer_protocol::kCodecParamRtxTime, &rtx_time) && rtx_time > 0) {
      ...
      case libmedia_transfer_protocol::VideoCodec::CODEC_VIDEO: {
  ...
  for (const auto& entry : rtx_mapping) {
    ...
    const libmedia_transfer_protocol::VideoCodec::CodecType associated_codec_type = it->second;
    if (associated_codec_type != libmedia_transfer_protocol::VideoCodec::CODEC_VIDEO &&
        associated_codec_type != libmedia_transfer_protocol::VideoCodec::CODEC_RED) {
      RTC_LOG(LS_ERROR)
...
std::vector<libmedia_codec::VideoStream> EncoderStreamFactory::CreateEncoderStreams(
    int width,
    int height,
    const libmedia_codec::VideoEncoderConfig& encoder_config) {
  RTC_DCHECK_GT(encoder_config.number_of_streams, 0);
  ...
  const absl::optional<webrtc::DataRate> experimental_min_bitrate =
	  webrtc::GetExperimentalMinVideoBitrate((webrtc::VideoCodecType)encoder_config.codec_type);

  if (encoder_config.number_of_streams > 1 ||
      ((absl::EqualsIgnoreCase(codec_name_, libmedia_transfer_protocol::kVp8CodecName) ||
        absl::EqualsIgnoreCase(codec_name_, libmedia_transfer_protocol::kH264CodecName)) &&
...
EncoderStreamFactory::CreateDefaultVideoStreams(
  ...
  int max_framerate = (encoder_config.simulcast_layers[0].max_framerate > 0)
                          ...
                          : libmedia_transfer_protocol::kDefaultVideoMaxFramerate;

  libmedia_codec::VideoStream layer;
  layer.width = width;
  layer.height = height;
  ...
  if (absl::EqualsIgnoreCase(codec_name_, libmedia_transfer_protocol::kVp9CodecName)) {
...
EncoderStreamFactory::CreateSimulcastOrConferenceModeScreenshareStreams(
  ...
  const bool has_scale_resolution_down_by = absl::c_any_of(
      encoder_config.simulcast_layers, [](const libmedia_codec::VideoStream& layer) {
