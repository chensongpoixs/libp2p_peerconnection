...
libmedia_transfer_protocol::RtpParameters CreateRtpParametersWithOneEncoding() {
	libmedia_transfer_protocol::RtpParameters parameters;
	libmedia_transfer_protocol::RtpEncodingParameters encoding;
...
libmedia_transfer_protocol::RtpParameters CreateRtpParametersWithEncodings(libmedia_transfer_protocol::StreamParams sp)
{
  ...
  std::vector<libmedia_transfer_protocol::RtpEncodingParameters> encodings(encoding_count);
  for (size_t i = 0; i < encodings.size(); ++i) {
    encodings[i].ssrc = primary_ssrcs[i];
  }

  const std::vector<libmedia_transfer_protocol::RidDescription>& rids = sp.rids();
...
std::vector<libmedia_transfer_protocol::RtpExtension> GetDefaultEnabledRtpHeaderExtensions(
  ...
  for (const auto& entry : query_interface.GetRtpHeaderExtensions()) {
    if (entry.direction != libmedia_transfer_protocol::RtpTransceiverDirection::kStopped)
...
webrtc::RTCError CheckRtpParametersValues(
    const libmedia_transfer_protocol::RtpParameters& rtp_parameters) {
...
webrtc::RTCError CheckRtpParametersInvalidModificationAndValues(
    const libmedia_transfer_protocol::RtpParameters& old_rtp_parameters,
    const libmedia_transfer_protocol::RtpParameters& rtp_parameters) {
  using webrtc::RTCErrorType;
  ...
  if (!absl::c_equal(old_rtp_parameters.encodings, rtp_parameters.encodings,
                     [](const libmedia_transfer_protocol::RtpEncodingParameters& encoding1,
                        const libmedia_transfer_protocol::RtpEncodingParameters& encoding2) {
