...
webrtc::DataRate GetTotalMaxBitrate(
    const std::vector<libmedia_codec::VideoStream>& layers) {
  if (layers.empty())
...
std::vector<libmedia_codec::VideoStream> GetSimulcastConfig(
    size_t min_layers,
    size_t max_layers,
    int width,
    int height,
...
std::vector<libmedia_codec::VideoStream> GetNormalSimulcastLayers(
    size_t layer_count,
    int width,
    int height,
    double bitrate_priority,
    ...
    const webrtc::WebRtcKeyValueConfig& trials) {
  std::vector<libmedia_codec::VideoStream> layers(layer_count);
  ...
  for (size_t s = layer_count - 1;; --s) {
    ...
    if (s == 0) {
      ...
      if (num_temporal_layers == 3) {
        ...
        rate_factor =
			libmedia_codec::SimulcastRateAllocator::GetTemporalRateAllocation(
    ...
    layers[s].max_framerate = libmedia_transfer_protocol::kDefaultVideoMaxFramerate;
...
std::vector<libmedia_codec::VideoStream> GetScreenshareLayers(
    size_t max_layers,
    int width,
    int height,
    double bitrate_priority,
  ...
  std::vector<libmedia_codec::VideoStream> layers(num_simulcast_layers);
  ...
  if (num_simulcast_layers == kMaxScreenshareSimulcastLayers) {
    ...
    layers[1].max_framerate = libmedia_transfer_protocol::kDefaultVideoMaxFramerate;
