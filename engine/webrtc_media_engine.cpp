...
#include "libp2p_peerconnection/engine/webrtc_voice_engine.h"
...
std::unique_ptr<MediaEngineInterface> CreateMediaEngine(
  ...
  auto audio_engine = nullptr;/*std::make_unique<WebRtcVoiceEngine>(
...
void DiscardRedundantExtensions(
    std::vector<libmedia_transfer_protocol::RtpExtension>* extensions,
  ...
  for (const char* uri : extensions_decreasing_prio) {
    auto it = absl::c_find_if(
        *extensions,
        [uri](const libmedia_transfer_protocol::RtpExtension& rhs) { return rhs.uri == uri; });
...
bool ValidateRtpExtensions(
  ...
  bool id_used[1 + libmedia_transfer_protocol::RtpExtension::kMaxId] = {false};
  for (const auto& extension : extensions) {
    if (extension.id < libmedia_transfer_protocol::RtpExtension::kMinId ||
        extension.id > libmedia_transfer_protocol::RtpExtension::kMaxId) {
...
std::vector<libmedia_transfer_protocol::RtpExtension> FilterRtpExtensions(
  ...
  std::vector<libmedia_transfer_protocol::RtpExtension> result;
  ...
  absl::c_sort(result, [](const libmedia_transfer_protocol::RtpExtension& rhs,
                          const libmedia_transfer_protocol::RtpExtension& lhs) {
  ...
  if (filter_redundant_extensions) {
    auto it = std::unique(
        ...
        [](const libmedia_transfer_protocol::RtpExtension& rhs, const libmedia_transfer_protocol::RtpExtension& lhs) {
...
libmedia_transfer_protocol::BitrateConstraints GetBitrateConfigForCodec(const libmedia_transfer_protocol::Codec& codec) {
	libmedia_transfer_protocol::BitrateConstraints config;
  int bitrate_kbps = 0;
  if (codec.GetParam(libmedia_transfer_protocol::kCodecParamMinBitrate, &bitrate_kbps) &&
      bitrate_kbps > 0) {
    config.min_bitrate_bps = bitrate_kbps * 1000;
  } else {
    config.min_bitrate_bps = 0;
  }
  if (codec.GetParam(libmedia_transfer_protocol::kCodecParamStartBitrate, &bitrate_kbps) &&
      bitrate_kbps > 0) {
  ...
  if (codec.GetParam(libmedia_transfer_protocol::kCodecParamMaxBitrate, &bitrate_kbps) &&
      bitrate_kbps > 0) {
