...
#ifndef _C_PC_JSEP_TRANSPORT_H_
#define _C_PC_JSEP_TRANSPORT_H_
...
#include "libp2p_peerconnection/sctp_transport.h"
#include "libp2p_peerconnection/csession_description.h"
//#include "libp2p_peerconnection/srtp_filter.h"
...
//class DtlsTransportInternal;
	...
	public:
		...
		JsepTransportDescription(
			...
			const libice::TransportDescription& transport_description);
		...
		libice::TransportDescription transport_desc;
	};
	...
	public:
		...
		//JsepTransport(const JsepTransport &);
		~JsepTransport();
		...
		webrtc::RTCError SetRemoteJsepTransportDescription(
			const JsepTransportDescription& jsep_description,
			webrtc::SdpType type);
		webrtc::RTCError AddRemoteCandidates(const libice::Candidates& candidates);
		...
		libp2p_peerconnection::RtpTransportInternal* rtp_transport() const {
		...
		const libice::DtlsTransportInternal* rtp_dtls_transport() const {
		...
		const libice::DtlsTransportInternal* rtcp_dtls_transport() const {
			RTC_DCHECK_RUN_ON(network_thread_);
		...
		rtc::scoped_refptr<DtlsTransport> RtpDtlsTransport() {
	...
	private:
		...
		bool SetSdes(const std::vector<cricket::CryptoParams>& cryptos,
		...
		void SetRemoteIceParameters(const libice::IceParameters& ice_parameters,
		...
		bool GetTransportStats(libice::DtlsTransportInternal* dtls_transport,
			int component,
		...
		const rtc::scoped_refptr<libice::IceTransportInterface> ice_transport_;
		const rtc::scoped_refptr<libice::IceTransportInterface> rtcp_ice_transport_;
		...
		const rtc::scoped_refptr<DtlsTransport> rtp_dtls_transport_;
		...
		rtc::scoped_refptr<DtlsTransport> rtcp_dtls_transport_
			RTC_GUARDED_BY(network_thread_);
		// TODO@chensong 2025-09-25   not datachannel 
		//const std::unique_ptr<webrtc::DataChannelTransportInterface>
		//	sctp_data_channel_transport_;
		const rtc::scoped_refptr< libp2p_peerconnection::SctpTransport> sctp_transport_;
