...
//#include "libmedia_transfer_protocol/sctp/sctp_data_channel_transport.h"
...
JsepTransportDescription::JsepTransportDescription(
    ...
    const libice::TransportDescription& transport_desc)
...
JsepTransportDescription::JsepTransportDescription(
	const JsepTransportDescription& from) = default;
	/*
...
JsepTransport::JsepTransport(
    ...
    rtc::scoped_refptr<libice::IceTransportInterface> ice_transport,
    rtc::scoped_refptr<libice::IceTransportInterface> rtcp_ice_transport,
    ...
    : network_thread_(rtc::Thread::Current()),
      ...
      sctp_transport_(sctp_transport
                          ? rtc::make_ref_counted<libp2p_peerconnection::SctpTransport>(
...
//JsepTransport::JsepTransport(const JsepTransport &) = default;
...
webrtc::RTCError JsepTransport::SetLocalJsepTransportDescription(
  ...
  libice::IceParameters ice_parameters =
  ...
  bool ice_restarting =
     ...
     libice:: IceCredentialsChanged(local_description_->transport_desc.ice_ufrag,
...
libice:: IceParameters ice_parameters =
  ...
  // If PRANSWER/ANSWER is set, we should decide transport protocol type.
  if (type == webrtc::SdpType::kPrAnswer || type == webrtc::SdpType::kAnswer) {
    error = NegotiateAndSetDtlsParameters(webrtc::SdpType::kOffer);
  }
  if (!error.ok()) {
    remote_description_.reset();
    return error;
...
webrtc::RTCError JsepTransport::AddRemoteCandidates(
  ...
  for (const libice::Candidate& candidate : candidates) {
    auto transport =
        candidate.component() == libice::ICE_CANDIDATE_COMPONENT_RTP
            ? rtp_dtls_transport_
...
bool JsepTransport::GetStats(TransportStats* stats) {
  ...
  bool ret = GetTransportStats(rtp_dtls_transport_->internal(),
                               libice::ICE_CANDIDATE_COMPONENT_RTP, stats);

  if (rtcp_dtls_transport_) {
    RTC_DCHECK(rtcp_dtls_transport_->internal());
    ret &= GetTransportStats(rtcp_dtls_transport_->internal(),
		libice::ICE_CANDIDATE_COMPONENT_RTCP, stats);
  }
  return ret;
}
...
void JsepTransport::SetRemoteIceParameters(
    const libice::IceParameters& ice_parameters,
...
webrtc::RTCError JsepTransport::SetNegotiatedDtlsParameters(
	libice::DtlsTransportInternal* dtls_transport,
...
bool JsepTransport::SetSdes(const std::vector<cricket::CryptoParams>& cryptos,
...
*/
  ...
  if ((type == webrtc::SdpType::kPrAnswer || type == webrtc::SdpType::kAnswer) && ret) 
  {
#if 0
...
webrtc::RTCError JsepTransport::NegotiateAndSetDtlsParameters(
  ...
  } else if (local_fp && (local_description_type == webrtc::SdpType::kAnswer)) {
...
webrtc::RTCError JsepTransport::NegotiateDtlsRole(
	webrtc::SdpType local_description_type,
    libice::ConnectionRole local_connection_role,
    libice::ConnectionRole remote_connection_role,
    absl::optional<rtc::SSLRole>* negotiated_dtls_role) {
  ...
  if (local_description_type == webrtc::SdpType::kOffer) {
    if (local_connection_role != libice::CONNECTIONROLE_ACTPASS) {
      return webrtc::RTCError(
          webrtc::RTCErrorType::INVALID_PARAMETER,
          "Offerer must use actpass value for setup attribute.");
    }

    if (remote_connection_role == libice::CONNECTIONROLE_ACTIVE ||
        remote_connection_role == libice::CONNECTIONROLE_PASSIVE ||
        remote_connection_role == libice::CONNECTIONROLE_NONE) {
      is_remote_server = (remote_connection_role == libice::CONNECTIONROLE_PASSIVE);
    } else {
      return webrtc::RTCError(
          webrtc::RTCErrorType::INVALID_PARAMETER,
    ...
    if (remote_connection_role != libice::CONNECTIONROLE_ACTPASS &&
        remote_connection_role != libice::CONNECTIONROLE_NONE) {
      ...
      if (!current_dtls_role) {
		...
		case libice::CONNECTIONROLE_ACTIVE:
            if (local_connection_role != libice::CONNECTIONROLE_PASSIVE) {
              return webrtc::RTCError(
                  webrtc::RTCErrorType::INVALID_PARAMETER,
          ...
          case libice::CONNECTIONROLE_PASSIVE:
            if (local_connection_role != libice::CONNECTIONROLE_ACTIVE) {
              return webrtc::RTCError(
                  webrtc::RTCErrorType::INVALID_PARAMETER,
        ...
        if ((*current_dtls_role == rtc::SSL_CLIENT &&
             remote_connection_role == libice::CONNECTIONROLE_ACTIVE) ||
    ...
    if (local_connection_role == libice::CONNECTIONROLE_ACTIVE ||
        local_connection_role == libice::CONNECTIONROLE_PASSIVE) {
      is_remote_server = (local_connection_role == libice::CONNECTIONROLE_ACTIVE);
    } else {
      return webrtc::RTCError(
          webrtc::RTCErrorType::INVALID_PARAMETER,
          "Answerer must use either active or passive value "
...
bool JsepTransport::GetTransportStats(libice::DtlsTransportInternal* dtls_transport,
                                      int component,
